...
class CSVScan(object):
    ...
    def __init__(self, filepath, schema: Schema):
        ...
        with open(self.filepath, "rb") as f:
            ...
            self.offset = f.tell()
        assert set(self.schema.columns.keys()) == set(self.header), (
            "column names in the schema should be the same as column names in the csv file"
        )
            ...
            try:
                ...
                if self.schema:
                    for column_name, column_type in self.schema.columns.items():
                        ...
                        final_row[f"{self.schema.table_name}.{column_name}"] = (
                            self.cast_variable(column_type, row[value_index])
                        )
                return final_row
...
class HeapScan(object):
    ...
    def get_row_index(self):
        start_range = int.from_bytes(
            self.page_data[(self.row_index + 1) * 2 : (self.row_index + 2) * 2]
        )
        end_range = encodingCsv.PAGE_SIZE
        if self.row_index != 0:
            end_range = int.from_bytes(
                self.page_data[self.row_index * 2 : (self.row_index + 1) * 2]
            )
    ...
    def decode_row(self, start_range, end_range):
        ...
        for column_name, datatype in self.schema.columns.items():
            ...
            if datatype == "int32":
                ...
                start_index += 2
                value = row_data[start_index : start_index + len_of_text].decode(
                    "utf-8"
                )
...
class Insert(object):
    ...
    def get_last_page_index(self):
        ...
        assert file_size % PAGE_SIZE == 0, (
            f"the file size is {file_size} should  multiple of PAGE_SIZE"
        )
    ...
    def write_data(self, start_index, end_index, value):
        ...
        """
        assert start_index <= end_index and end_index <= PAGE_SIZE, (
            "wronge values for indexes in page writing  "
        )
    ...
    def update_page_header(self, refernce: int):
        ...
        self.write_data(
            number_of_rows * 2, number_of_rows * 2 + 2, refernce.to_bytes(2)
        )
    ...
    def get_new_ranges(self, length_of_row):
        ...
        current_data = self.get_data()
        last_index_used = int.from_bytes(
            current_data[reference_index : reference_index + 2]
        )
...
class NestedLoopJoin(object):
    ...
    def next(self):
        assert self.childs[0] or self.childs[1], (
            "the join must have atleast two childrens"
        )
...
class SortMergeJoin(object):
    ...
    def next(self):
        while True:
            if self.is_finished:
                return None
            if self.last_left == self.left_row and self.last_left_result:
                row = self.last_left_result.pop()
            ...
            if action == SortMergeActions.GET_LEFT_MATCH:
                row = self.childs[0].next()
                self.is_finished = row is None
                result: dict = self.left_row.copy()
                result.update(self.right_row)
                self.current_left_result.append(result)
                self.last_left_result = self.current_left_result
                self.current_left_result = []
                self.last_left = self.left_row
                self.left_row = row
                return result

            elif action == SortMergeActions.GET_RIGHT_MATCH:
                row = self.childs[1].next()
                self.is_finished = row is None
                result: dict = self.left_row.copy()
                result.update(self.right_row)
                self.right_row = row
            ...
            elif action == SortMergeActions.GET_LEFT_NO_MATCH:
                row = self.childs[0].next()
                self.is_finished = row is None
                self.last_left = self.left_row
                self.left_row = row
            ...
            elif action == SortMergeActions.GET_RIGHT_NO_MATCH:
                row = self.childs[1].next()
                self.is_finished = row is None
                self.right_row = row
            else:
                raise ValueError(f"there is no action Code with this value: {action} ")
        ...
        run(
            QueryBuilder(
                [
                    GroupBy(
                        lambda x: x[3],
                        lambda key, records: (key, sum(r[2] for r in records)),
                    ),
                    Sort(lambda x: x[3]),
                    MemoryScan(birds),
                ],
...
def test_group_by_node():
    ...
    x = tuple(
        run(
            QueryBuilder(
                [
                    GroupBy(lambda _: 1, lambda key, records: (key, len(records))),
...
def test_insert_functionalty():
    ...
    linked_list = tuple(
        run(QueryBuilder([Insert(file_path, schema), MemoryScan(table)], [-1, 0]))
    )
    ...
    table_after_insert = tuple(run(QueryBuilder([HeapScan(file_path, schema)], [-1])))
    length_of_table_after_insert = len(table_after_insert)


def test_order_heap_file_reader():
    csv_file_path = (
        "/Users/ahmeali/Downloads/ml-20m/movies.csv"  # Path to your CSV file
    )
    ...
    x = tuple(run(QueryBuilder([HeapScan(heap_file_path, schema)], [-1])))
    wanted_output = x[-2:]
